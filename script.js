...
function calcularPatron() {
    ...
    if (tipoPrenda === 'CUBRE_PAÑAL') {
        ...
        // 1. CÁLCULO DE PUNTOS Y HOLGURA DE CUERPO (INFERIDO/EXISTENTE)
        // Holgura de pecho/cuerpo es 5% del CP o 5cm, el menor
        const holguraCm = Math.min(medidas.CP * 0.05, 5.0);
        const anchoPrendaCm = medidas.CP + holguraCm;
        const cpPts = Math.round(anchoPrendaCm * densidadP);

        // Variables comunes
        const tiraCuelloCm = 1.5; 
        const tiraCuelloPts = densidadH ? Math.round(tiraCuelloCm * densidadH) : 0;
        const puntosTapeta = densidadP ? Math.round(tiraCuelloCm * densidadP) : 0;
        const ccAjustadoCm = medidas.CC + (tiraCuelloCm * 2); // Para Raglán
        const ccPts = Math.round(ccAjustadoCm * densidadP); // Para Raglán
        const raglanCmBase = 1.5; // Base para raglan top-down (e.g., 1.5cm)
        ...
        if (metodoTejido === "BAJO") {
            ...
            // --- 0. CÁLCULO DE SISA Y HOLGURA DE MANGA (Nueva Lógica solicitada) ---
            let holguraMangaCm;
            ...
            // Tallas de Adulto (36 a 50)
                ...
                // Holgura por defecto (Adulto)
                holguraMangaCm = 2.5; 
            }
            
            // El Contorno de Axila (CA) viene de las medidas
            const CA = medidas.CA; 

            // Cálculo del ancho de sisa (Contorno de Axila / 2 + Holgura)
            const anchoSisaMangaCm = (CA / 2) + holguraMangaCm;
            ...
            // const puntosSisaManga está calculado arriba con holgura (NUEVA LÓGICA)
