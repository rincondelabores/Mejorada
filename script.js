...
// ====================================================================
// 3. LÓGICA CENTRAL DE CÁLCULO (FUNCIÓN CORREGIDA)
// ====================================================================
...
function calcularPatron() {
    ...
    if (tipoPrenda === 'CUBRE_PAÑAL') {
        ...
        // ******************************************************************
        const holguraCm = 6; 
        const tiraCuelloCm = 2; // Ancho de la tira de cuello (ej. 2cm)
        const raglanCmBase = medidas.PSisa; // Profundidad de sisa
        
        const tiraCuelloPts = Math.round(tiraCuelloCm * densidadP);
        
        // Contorno y puntos base
        const anchoPrendaCm = medidas.CP + holguraCm; // Contorno de pecho + holgura
        const cpPts = Math.round(anchoPrendaCm * densidadP); // Puntos de contorno de pecho total
        const ccAjustadoCm = medidas.CC - tiraCuelloCm * 2; // Contorno de cuello ajustado
        const ccPts = Math.round(ccAjustadoCm * densidadP); // Puntos de contorno de cuello
        
        // Tapeta (solo para Chaqueta)
        const puntosTapeta = Math.max(3, tiraCuelloPts); 

        // Se define el ancho de la sisa/manga
        const holguraMangaCm = 4;
        
        // ** FIX SOLICITADO POR LA USUARIA: (CA/2) + holgura para el ANCHO de la manga en plano **
        const anchoSisaMangaCm = (medidas.CA / 2) + holguraMangaCm;
    ...
    resultadoDiv.innerHTML = resultado.replace(/\*\*(.*?)\*\*/g, '<b>$1');
}
