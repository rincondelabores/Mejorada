...
function calcularPatron() {
    ...
    if (isNaN(puntosMuestra) || puntosMuestra <= 0) {
        // CAMBIO: Debe introducir -> Debes introducir
        resultadoDiv.innerHTML = '<p class="error">Error: Debes introducir los **puntos de la muestra** de tensión (en 10 cm).</p>';
        return;
    }
    if (!tipoPrenda) {
        // CAMBIO: Debe seleccionar -> Debes seleccionar
        resultadoDiv.innerHTML = '<p class="error">Error: Debes seleccionar un **tipo de prenda**.</p>';
        return;
    }
    ...
    if (tipoPrenda === 'CUBRE_PAÑAL') {
        ...
        resultado += `<p>Se teje de una sola pieza (desde la espalda hasta el delantero).</p>\n`;

        resultado += `\n<u>A tener en cuenta antes de empezar a tejer:</u>\n`;
        resultado += `* **Cinturilla:** En la cintura sugerimos tejer  **${elásticoCm} cm** ** en punto elástico** o añadir una vuelta de **pasacintas** cuando estés tejiendo en la mitad de esos cm (Pasacintas:*2p juntos al derecho, hebra*, repetir de *a* toda la pasada).  El ancho es orientativo, puedes hacerlo mas ancho o estrecho, segun te guste.\n`;
        resultado += `* **Bordes:** Para un borde rematado al mismo tiempo que se teje la pierna, haz los aumentos y disminuciones a 3 o 4 puntos de la orilla. Otra opción es recoger después los puntos del borde de las piernas y para tejer una tira de terminación en el punto que te guste.
        \n`;

        
        resultado += `<u>Manos a la obra!:</u>\n`;
        resultado += `* **Monta:** **${puntosMontar} puntos** (corresponde a ${CC / 2} cm de cintura).
        \n`;
        ...
        // 1. Altura Lateral
        resultado += `* ** Teje recto **${AL} cm** ${hilerasAL !== null ? `(**${hilerasAL} pasadas**)` : ''}.\n`;
        
        // 2. Línea de Disminuciones
        resultado += `* **Ahora has llegado al muslo y tienes que empezar a disminuir:** Disminuye **${menguadosPorLado} veces 1 punto a cada lado.**\n`;
        ...
        // 3. Tramo Recto
        resultado += `* **Has llegado a la entrepierna, ahora tienes que hacer unas pasadas rectas antes de empezar el delantero :** Teje recto los **${puntosEP} puntos** durante **${TR} cm** ${hilerasTR !== null ? `(**${hilerasTR} pasadas**)` : ''}.\n\n`;
        ...
        resultado += `* **Acabados los aumentos**, antes de empezar a tejer las últimas pasadas para llegar a la cintura **tendrás que aumentar** de una vez **${puntosLCD} puntos a cada lado**. (Tendrás **${puntosMontar} puntos, los mismos puntos con los que empezaste a tejer**).\n`;
        
        // 6. Altura Lateral Delantero
        resultado += `* ahora **continúa tejiendo** recto **${AL} cm** ${hilerasAL !== null ? `(**${hilerasAL} pasadas**)` : ''}.\n`;
        
        // 7. Cierre y Costura
        resultado += `\n<u>3. Acabado</u>\n`;
        resultado += `* ** Cierra los **${puntosMontar} puntos** de la cintura delantera.\n`;
        resultado += `* **Cose** los laterales.\n`;
        ...
        // ... Variables de holgura y cuello ...
        const HOLGURA_CM = 5.0; 
        const TIRA_CUELLO_CM = 3.0; // Tira del cuello (para caída)

        // Variables de la talla
        const cpPts = Math.round((medidas.CP + HOLGURA_CM) * densidadP);
        const anchoPrendaCm = medidas.CP + HOLGURA_CM;
        const ccAjustadoCm = medidas.CC + HOLGURA_CM;
        const ccPts = Math.round(ccAjustadoCm * densidadP);

        const tiraCuelloPts = Math.round(TIRA_CUELLO_CM * densidadP);
        const raglanCmBase = medidas.CA;
        
        const holguraMangaCm = 3.0; 
        const anchoSisaMangaCm = medidas.AE + holguraMangaCm;
        const puntosSisaManga = Math.round(anchoSisaMangaCm * densidadP);
        
        let puntosTapeta = 0;
        let tiraCuelloCm = TIRA_CUELLO_CM;
        if (tipoPrenda === "CHAQUETA") {
            puntosTapeta = Math.round(TIRA_CUELLO_CM * densidadP);
        }
        ...
        if (metodoTejido === "BAJO") {
            ...
            resultado += `* **Monta:** **${puntosEspalda} puntos**.\n`;
            resultado += `* **Teje hasta la Sisa:** **${largoCuerpoCm.toFixed(1)} cm** ${hilerasBajoSisa !== null ? `(**${hilerasBajoSisa} pasadas**)` : ''}.\n`; 
            resultado += `* **Continúa tejiendo de Sisa a Hombro (Recto):** **${medidas.PSisa.toFixed(1)} cm** ${hilerasSisaHombro !== null ? `(**${hilerasSisaHombro} pasadas**)` : ''}.\n`; 
            resultado += `* **Total Tejido (De bajo a Hombro):** **${medidas.LT.toFixed(1)} cm** ${hilerasTotalEspalda !== null ? `(**${hilerasTotalEspalda} pasadas**)` : ''}. Cierra todos los puntos al finalizar.\n\n`;

            // 2. DELANTERO(S)
            resultado += `<u>2. Delantero(s)</u>\n`;
            if (tipoPrenda === "JERSEY") {
                resultado += `* **Monta:** **${puntosTotalDelantero} puntos**.\n`;
            } else { // CHAQUETA
                resultado += `* **Monta:** **${puntosTotalDelantero} puntos** (por cada Delantero).\n`;
            ...
            resultado += `* **Teje hasta la Sisa:** **${largoCuerpoCm.toFixed(1)} cm** ${hilerasBajoSisa !== null ? `(**${hilerasBajoSisa} pasadas**)` : ''} (igual que la espalda).\n`; 
          
            // INSTRUCCIONES DE ESCOTE
            resultado += `<u>Indicacciones para el Escote (Delantero)</u>\n`;
            resultado += `* **1. Teje el Escote ** a los **${escoteCmDesdeSisa.toFixed(1)} cm** desde el inicio de la sisa. ${hilerasInicioEscote !== null ? `(En la pasada **${hilerasInicioEscote}**).` : ''}\n`;
            
            if (tipoPrenda === "JERSEY") {
                 resultado += `* **2. Cierre Central (Recto):** Cerrar los **${puntosEscoteCentral} puntos** centrales. Esto divide el tejido en dos lados.\n`;
                 resultado += `* **3. Curva de Escote (Ambos lados):** Continúa tejiendo y cierra en el borde del escote de la siguiente manera: **${cierresEscote.join(', ')}** (un total de **${puntosAFormarEscotePts} puntos** a cerrar por lado).\n`;
                 resultado += `* **4.  Hombro:** Continúa recto los **${cmRectoOutput} cm** ${hilerasRestantesStr} restantes. Cierra los **${puntosHombro} puntos** restantes por hombro al llegar a la altura total de sisa (**${medidas.PSisa.toFixed(1)} cm** ${hilerasSisaHombro !== null ? `(**${hilerasSisaHombro} pasadas**)` : ''}).\n\n`; 
            } else { // CHAQUETA
                ...
                resultado += `* **2. Cierre Central (Escote):** Cerrar **${puntosCierreInicial} puntos**${avisoTapetaEnCierre} y luego continúa disminuyendo de la siguiente manera: **${secuenciaTotal.join(', ')}** (un total de **${totalCierreLateral} puntos** a disminuir).\n`;
                resultado += `* **3. Hombro:** Continúa recto y cierra los **${puntosHombro} puntos** restantes en el hombro al llegar a los **${medidas.PSisa.toFixed(1)} cm** de altura total de sisa ${hilerasSisaHombro !== null ? `(**${hilerasSisaHombro} pasadas**)` : ''}.\n\n`; 
            }
            ...
            resultado += `* **Monta:** **${puntosPuño} p.** (Puño de **${medidas['C Puño'].toFixed(1)} cm**).\n`;
            resultado += `* **Teje:** **${largoMangaSisaPuñoCm.toFixed(1)} cm** (Largo de Sisa a Puño). ${largoMangaH !== null ? `(**${largoMangaH} pasadas**)` : ''}\n`;
            ...
            if (aumentosPorLado > 0) {
                ...
                resultado += `* **Aumentos:** Aumenta **1 punto a cada lado** **${aumentosPorLado} veces** con una frecuencia de **${frecuenciaStr}**. Esto lleva la manga a **${puntosSisaManga} puntos** (**${cmSisaFinal} cm** de contorno en la sisa, incluyendo **${holguraMangaCm.toFixed(1)} cm** de holgura).\n\n`;
            } else {
        ...
        // --- LÓGICA TOP-DOWN (Escote al Bajo - Raglán) ---
            ...
            resultado += `* **Monta:** **${puntosMontaje} puntos** (**${ccAjustadoCm.toFixed(1)} cm** de contorno).\n`;
            resultado += `* **A continuación:** Teje **${tiraCuelloPts} pasadas** (**${tiraCuelloCm.toFixed(1)} cm**) para la tira del cuello.\n`;
            ...
            if (densidadH) {
                 instruccionRaglanStr = `Aumenta 1 punto a cada lado de los 4 marcadores (8 aumentos total) cada **2 pasadas** hasta completar **${hilerasRaglan} pasadas**.\n`;
            ...
            resultado += `* ** Deja el Cuerpo en espera. Pon los **${puntosMangaFinal_PreSisa} puntos** de la manga a una aguja de trabajo.\n`;
            ...
            if (puntosAnadirSisaPts % 2 === 0 && puntosAnadirSisaPts > 0) {
                ...
                resultado += `<p style="font-size:0.9em; padding-left: 20px;">* **Nota (Agujas rectas):** Si tejes la manga en plano (con costura), debes dividir los **${puntosAnadirSisaPts} puntos** de la sisa en dos: **${mitadPuntosSisa} puntos** antes de la manga y **${mitadPuntosSisa} puntos** después de la manga.</p>\n`;
            }
            ...
            if (vecesDisminuir > 0) {
                ...
                resultado += `<p style="padding-left: 20px;">- Disminuye **1 punto a cada lado** **${vecesDisminuir} veces** **${frecuenciaStr}**.\n`;
            ...
            if (tipoPrenda === "JERSEY") {
                ...
                resultado += `- **Puntos Totales:** Continúa tejiendo con un total de **${puntosTotalCuerpoFinal} puntos**.\n`;
                
                if (puntosAnadirSisaPts % 2 === 0 && puntosAnadirSisaPts > 0) {
                     const mitadPuntosSisa = puntosAnadirSisaPts / 2;
                     // CAMBIO: Si prefiere tejer -> Si prefieres tejer, recuerde añadir -> recuerda añadir
                     resultado += `<p style="font-size:0.9em; padding-left: 20px;">* **Tejido Separado (Plano):** Si prefieres tejer el Delantero y la Espalda por separado, recuerda añadir los **${puntosAnadirSisaPts} puntos** bajo manga divididos en dos: **${mitadPuntosSisa} puntos** al inicio y final de la Espalda y **${mitadPuntosSisa} puntos** al inicio y final del Delantero.</p>\n`;
                } else {
                     // CAMBIO: Si prefiere tejer -> Si prefieres tejer, recuerde añadir -> recuerda añadir
                     resultado += `<p style="font-size:0.9em; padding-left: 20px;">* **Tejido Separado (Plano):** Si prefieres tejer el Delantero y la Espalda por separado, recuerda añadir los **${puntosAnadirSisaPts} puntos** bajo manga como puntos de montaje/aumento al inicio y final de la Espalda y al inicio y final del Delantero.</p>\n`;
                }
            ...
            resultado += `* **Largo del Cuerpo (desde Sisa a Bajo)::** Continúa recto **${finalLargoCuerpoCm} cm** ${largoCuerpoRestanteH !== null ? `(**${largoCuerpoRestanteH} pasadas**)` : ''}.\n`;

        } else {
             // Si es Jersey/Chaqueta y falta el método
            resultadoDiv.innerHTML = '<p class="error">Error: Por favor, completa todos los campos obligatorios: **Puntos de Muestra** y selección de **Talla** y **Tipo de Prenda** y **Método de Tejido**.</p>';
            return;
        }
    }
